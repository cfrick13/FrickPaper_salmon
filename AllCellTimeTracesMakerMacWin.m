...
[detailssheet,~,detailsraw] = xlsread(DetailsFilename,Date);

filename = strcat(Date,'*smad3g same cells*SHEET.xlsx');
filelist = dir(filename);
[~,sheets] = xlsfinfo(char({filelist.name}));
...
[spreadsheet,~,spreadraw] = xlsread(char({filelist.name}),svalue);
