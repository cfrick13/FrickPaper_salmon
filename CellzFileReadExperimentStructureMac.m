function CellzFileReadExperimentStructureMac
basallength=6;
fs=7;
tshift=0;
smoothing = 12;
...
% for chosp  = {'647_wnt9a','594_wnt9a','647_ctgf','594_ctgf'}
% for chosp  = {'594_snail','647_snail'}
for chosp  = udmat'
    close all
chosenspecies = char(chosp);
%%%%%%%%
ijij=1;
cd (DATAdir)
load(dataNameTwo);
...
%     disp(Comms);
    ...
    elseif strcmp(Dose,'Empty')
        ...
        [~,~,~,d] = regexp(fnames,chosenspecies);
        csidx = cellfun(@isempty,d,'UniformOutput',1);
        [~,~,~,d] = regexp(fnames,Position);
        posidx = cellfun(@isempty,d,'UniformOutput',1);
        idx = ~dateidx&~csidx&~posidx;
...
%         [CommentStruct,leg] = CommentsIntoLegend(TimeTraceAndDeets,cellNumbers,cellScalar,emmies);
        ...
        [CommentStruct,leg] = CommentsIntoLegendNew(TimeTraceAndDeets,cellNumbers,cellScalar,emmies,stringer,stringsarray);
        ...
        editPlotLineFormatting(h,emmies,CommentStruct,stringsarray)
        editPlotLineFormattingSecond(hh,emmies,CommentStruct,stringsarray)

        editPlotLineFormattingSecond(hth,emmies,CommentStruct,stringsarray)
        editPlotLineFormattingSecond(hthh,emmies,CommentStruct,stringsarray)

        editPlotLineFormattingSecond(hhh,emmies,CommentStruct,stringsarray)
...
%         end
        ...
        emmies = emrs;
                ...
                scalarMat(end+1,:) = emmies';
                    ...
                    scalarMat(end+1,:) = emmies;
                    disp(strcat(Date,'-',Position,'-mismatch'))
                end

        %%%COMMENT OUT TO INCLUDE ALL CELLS IN SCALAR DATA)
        stringsarrayexclude = stringsarray;
        [scalarMatSelectLogical,excluderLogicalCombo] = scalarMatDataExcluder(CommentStruct,stringsarrayexclude);
...
%         end
        ...
        %%%%ConcatenateScalars
        [a,b] = regexp(EmFileChar,'(594_snail|594_smad7|594_pai1|647_snail|647_wnt9a|647_ctgf|594_wnt9a|594_ctgf|647_smad7|647_pai1|594_bhlhe40)');
        GeneName = EmFileChar(a:b);

        GeneNamesPresent = fieldnames(GeneScalarStruct);
        
        if strcmp(Dose,'0dot03')
            disp(emmies)
            stophere=1;
        end
        %%%determine if structure exists for that gene name
...
function [scalarMatSelect,excluderLogicalCombo] = scalarMatDataExcluder(CommentStruct,excluderArray) %only keep non-overdrivers

for j = 1:length(excluderArray)
h=excluderArray{j};
b = isspace(h);
h(b)=[];
excluderLogical = CommentStruct.(h);
if j ==1
...
function [CommentStruct,leg] = CommentsIntoLegendNew(TimeTraceAndDeets,cellNumbers,cellScalar,emmies,stringer,stringsarray)
...
nanstr = cellfun(@isstr,cellComments,'UniformOutput',1);
cellComments(~nanstr) = {''};

% disp(cellComments)
 [o,oo] = regexpi(cellComments,stringer);
 Overlap = ~cellfun(@isempty,o);
 m = find(Overlap==1);
 commstring = cell(1,length(Overlap));
 for mm =m
     comm = cellComments{mm};
     commstring{mm} = comm(o{mm}:oo{mm});
 end
 commstring(~Overlap) = {''};
 legOver = repmat({''},size(Overlap,1),size(Overlap,2)); 
 legOver = commstring;
...
primo = strcat((num2str(prim)),'...',cellstr(legOver'));
leg = char(primo);

cumulativeComments = false(size(cellComments));
for i=1:length(stringsarray)
[o,oo] = regexpi(cellComments,stringsarray{i});
Overlap = ~cellfun(@isempty,o);
h=stringsarray{i};
b=isspace(h);
h(b)=[];

cumulativeComments = cumulativeComments+Overlap;


CommentStruct.(h) = Overlap;
end

a = regexp(cellComments(cumulativeComments==0),'cfp');
xtra = cellfun(@isempty,a);

if ~isempty(cumulativeComments==0) && sum(xtra)>0
%     disp(cellComments')
end
stophere=1;
end
...
CellComments = TimeTraceAndDeets.cellComments';
cellComments = CellComments(1:2:end);
% Overlap = regexpi(cellComments,'overlap')
Overlap = strcmpi(cellComments,'overlap');
OverDrivers = strcmpi(cellComments,'overdriver');
Dimmers = strcmpi(cellComments,'dimmer');
Remove = strcmpi(cellComments,'remove');
Big = strcmpi(cellComments,'big');
DoubleNuc = strcmpi(cellComments,'doublenuc');
    NospaceCFP = strcmpi(cellComments,'no cfp');
    NoCFP = strcmpi(cellComments,'nocfp');
NoCfp = logical(NospaceCFP+NoCFP);
Saturated = strcmpi(cellComments,'saturated');

CommentStruct.Overlap = Overlap;
...
primo = strcat((num2str(prim)),'...',cellstr(legOver'),' ',cellstr(legDim'),' ',cellstr(legRem'),' ',cellstr(legDub'),' ',cellstr(legNoC'),' ',cellstr(legSat'),' ',cellstr(legBig'),' ',cellstr(legLap'));
leg = char(primo);
end
...
function editPlotLineFormatting(h,emmies,CommentStruct,stringsarray)
hline = findobj(h, 'type', 'line');
set(hline(~isnan(emmies)),'LineWidth',3)
fnames = fieldnames(CommentStruct);
for string =fnames'
    h=char(string);
    b=isspace(h);
    string=h;
    string(b) = [];
set(hline(CommentStruct.(string)),'LineStyle',':')
end
end

function editPlotLineFormattingSecond(h,emmies,CommentStruct,stringsarray)
hline = findobj(h, 'type', 'line');
...
for string =fnames'
    ...
    string=h;
    string(b) = [];
set(hline(CommentStruct.(string)),'LineStyle','none')
end
...
diffcfp = nanmax(abs(diff(CFP_N(i,:))));
if abs(diffcfp)>0.4
    CFP_N(i,:)=ones(size(CFP_N(i,:))); 
end
% if stdcfp>0.20
%    CFP_N(i,:)=ones(size(CFP_N(i,:))); 
% end
