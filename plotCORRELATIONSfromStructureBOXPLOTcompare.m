...
% CountType = 'm2016-02-17'; %feb09-2016
% CountType = 'm2016-02-17-b'; %feb09-2016 more curation

% GeneChoiceArray = {'647_ctgf';'594_ctgf'};
% % GeneChoiceArray = {'647_wnt9a';'594_wnt9a'};
% CountType = 'm2016-02-19'; %feb19-2016 more curation
% 
% GeneChoiceArray = {'647_ctgf';'594_ctgf'};
% % GeneChoiceArray = {'647_wnt9a';'594_wnt9a'};
% CountType = 'onlym2016-02-19'; %only feb19-2016 more curation

% GeneChoiceArray = {'647_ctgf';'594_ctgf'};
% % GeneChoiceArray = {'647_wnt9a';'594_wnt9a'};
% CountType = 'm2016-02-19only09'; %feb19-2016 more curation
% CountType = 'm2016-02-19b'; %feb19-2016 more curation
...
% GeneChoiceArray = {'647_ctgf';'594_ctgf'};
GeneChoiceArray = {'647_wnt9a';'594_wnt9a'};
% CountType = 'mALLFOCUS'; %feb19-2016 more curation'mlatestcurated.mat'
CountType = 'mrna_all'; %feb19-2016 more curation'mlatestcurated.mat'
CountType = 'mrna_2016_02_19'; %feb19-2016 more curation'mlatestcurated.mat'
...
hht = HH{2};
hhhh = struct();
fnamesone = fieldnames(hho);
fnamestwo = fieldnames(hht);
ufnames = unique(vertcat(fnamesone,fnamestwo));
for fntstr = ufnames'
    fnt =char(fntstr);
    fidxo = strcmp(fnamesone,fnt);
    fidxt = strcmp(fnamestwo,fnt);
    if sum(fidxt)==1 && sum(fidxo)==1
       hhhh.(fnt) = horzcat(hho.(fnt),hht.(fnt));
    elseif sum(fidxt)==1 && sum(fidxo)==0
        hhhh.(fnt) = hht.(fnt);
    elseif sum(fidxt)==0 && sum(fidxo)==1
        hhhh.(fnt) = hho.(fnt);
    end
end
...
%             disp(DoseStr{i});
    ...
    Lowscal = sortedscal(round(length(sortedscal).*0.05));
    if Lowscal == 0
        axis('tight')
    else
 
        Lowms =  sortedms(round(length(sortedms).*0.02));
        if Lowms ==0
            Lowms =1;
        end
